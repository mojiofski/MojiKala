...
async function SingleProduct({ params }: ISingleProductProps) {
  ...
  try {
    const response = await fetch(
      `http://localhost:3001/smartwatches?name=${encodeURIComponent(
        productName
      )}`,
      { cache: "no-store" }
    );
    ...
    return (
      <div className="flex flex-col lg:flex-row mt-2 w-full bg-gray-100 min-h-screen lg:p-4">
        {/* Product Image */}
        ...
        {/* Product Information */}
        <div className="w-full lg:w-1/2 lg:p-4 bg-red-200 ">
